...
hey = opr.one._extend('test', func='callTrueTest', dat='extensions_for_tests', args=['I AM AN ARG THATS WORKING'], call=True, returnObj=True)
...
test = opr._extend('test', cls='myClass', dat='extensions_for_tests', returnObj=True)
initiate = test()
tf.info('Checking to make sure we can access "testFunc()" from extension class')
log(test.testFunc())
tf.info('checking call=True for class extension')
test = opr._extend('test', cls='myClass', dat='extensions_for_tests', args=['arg'], call=True, returnObj=True)
log(test.testFunc())
