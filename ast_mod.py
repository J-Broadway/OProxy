...
def Main(cls=None, func=None, op=None):
    ...
    # Type checking and handling for op
    op = td.op(op) if isinstance(op, str) else op
    if op is None or not isinstance(op, td.textDAT):
        raise ValueError(f"Provided op must be a td.textDAT or a string path to one; got {type(op)}")
